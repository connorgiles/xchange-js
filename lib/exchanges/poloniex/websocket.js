	...
	prototype.onMessage = function(data) {
		...
		_.each(data[2], d => {
			if (d[0] === 'i' && d[1] && d[1].currencyPair) {
				...
				let side = d[1] === 0 ? 'ask' : 'bid';
				d.push(side);
				let pp = parseLevel(_.slice(d, 2, 5));
 ...
 */
 function parseLevel(msg) {
 	return {id: parseFloat(msg[0]), price: parseFloat(msg[0]), amount: parseFloat(msg[1]), type: msg[2]}
 }
