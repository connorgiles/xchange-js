...
_.assign(WebsocketClient.prototype, new function() {
	...
	prototype.onMessage = function(data) {
		...
		// heartbeat
		if (cid === 1010) return;
		else if (0 >= cid || cid >= 1000) return self.onError('Wrong channel number', cid);
		...
		_.each(data[2], d => {
			if (d[0] === 'i' && d[1] && d[1].currencyPair) {
				...
				_.forIn(d[1].orderBook[1], (amount, price) => {
					let pp = adapters.socketPriceLevel([price, amount, 'bid']);
					if (p.book.get(pp.id)) p.book.change(pp);
					else p.book.add(pp);
				});
				_.forIn(d[1].orderBook[0], (amount, price) => {
					let pp = adapters.socketPriceLevel([price, amount, 'ask']);
					if (p.book.get(pp.id)) p.book.change(pp);
					else p.book.add(pp);
				});
			}
				...
				let pp = adapters.socketPriceLevel(_.slice(d, 2, 5));
