...
_.assign(WebsocketClient.prototype, new function() {
	...
	prototype.subscribeRaw = function(pair) {
		let self = this;
		...
		self.socket.send(JSON.stringify({ event: 'subscribe', channel: 'book', pair: pair, prec: 'R0', len: '100' }));
		let exchange = path.basename(__dirname);
		self.markets[pair] = new Market(pair, exchange);
		return self.markets[pair];
	};
	...
	prototype.onMessage = function(data) {
		...
		if (p.book.seq === 0) {
			_.each(data[1], (pp) => {
				p.book.add(parseLevel(pp));
			});
		} 
		else {
			let pp = parseLevel(data.slice(1));
			if (pp.price === 0) p.book.remove(pp);
			else {
				let order = p.book.get(pp.id);
 ...
 */
 function parseLevel(msg) {
 	return {id: msg[0], price: msg[1], amount:  Math.abs(msg[2]), type: msg[2] >= 0 ? 'bid' : 'ask'}
 }
